...
class OpenAPIRule(group: RuleGroup) : ValidationRule<OpenAPI>(group) {
    ...
    fun info(rule: InfoRule.() -> InfoRule): OpenAPIRule {
        add {
            rule(InfoRule(RuleGroup.named("info", group))).validate(it.info)
        }
        return this
    }
    ...
    fun paths(rule: PathsRule.() -> PathsRule): OpenAPIRule {
        add {
            rule(PathsRule(RuleGroup.named("paths", group))).validate(it.paths)
        }
        return this
    }
    ...
    fun components(rule: ComponentsRule.() -> ComponentsRule): OpenAPIRule {
        add {
            rule(ComponentsRule(RuleGroup.named("components", group))).validate(it.components)
        }
        return this
    }
}
...
fun openAPI(rule: OpenAPIRule.() -> OpenAPIRule): OpenAPIRule {
    return rule(OpenAPIRule(RuleGroup.named("document")))
}